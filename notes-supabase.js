...
function formatAuthor(name) {
    ...
    if (raw.trim().toLowerCase() === "kinqsy") {
...
async function countReactions(filter) {
    const { data, error } = await supabaseClient
        .from("reactions")
        .select("reaction")
        .match(filter);
...
async function addReaction(payload) {
    const { error } = await supabaseClient.from("reactions").insert(payload);
    if (error) {
...
function reactionsHtml(counts, kind, id) {
    return (
        '<div class="reactions" data-kind="' + kind + '" data-id="' + id + '">' +
            '<button type="button" class="react-btn" data-reaction="heart">❤️ ' + counts.heart + '</button>' +
            '<button type="button" class="react-btn" data-reaction="broken">💔 ' + counts.broken + '</button>' +
        '</div>'
    );
}
...
function bindReactions(root) {
    root.querySelectorAll(".reactions").forEach(function (box) {
